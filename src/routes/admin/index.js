...
const Agent = lazy(() => import('../../container/agents'));
const Charts = lazy(() => import('./charts'));
const KnowledgeBase = lazy(() => import('../../container/pages/knowledgeBase/Index'));
const Calendar = lazy(() => import('../../container/pages/calendar/index'));
const TaskDetails = lazy(() => import('../../container/pages/task/details/index'));
...
const Map = lazy(() => import('../../container/pages/map'));

const Admin = React.memo(() => {
  const { pathname } = useLocation();
  useEffect(() => {
    window.scrollTo(0, 0);
    // agregar login on refresh
  }, [pathname]);
  return (
    <Suspense
      fallback={
        <div className="spin flex items-center justify-center bg-transparent h-screen w-full fixed z-[40] ltr:left-0 rtl:right-0 top-0">
          <Spin />
        </div>
      }
    >
